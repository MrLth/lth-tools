...
exports.cancelRequest = exports.requestCreator = void 0;
/*
 * @Author: mrlthf11
...
axios_1.default.defaults.timeout = config_1.REQ_CFG.TIMEOUT;
...
function request(url, params, config) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var cacheDuration = (0, lodash_es_1.isNumber)(cache) ? cache : cache ? config_1.REQ_CFG.CACHE_TIME : 0;
    var cachePromiseOrWrite = RequestCache_1.default.read(url, params, cacheDuration);
    var promise;

    if (!(0, lodash_es_1.isFunction)(cachePromiseOrWrite)) {
      ...
      switch (reqMethod) {
        case 'get':
          promise = axios_1.default.get(reqUrl, Object.assign({
            params: params,
        ...
        case 'post':
          promise = axios_1.default.post(reqUrl, params, Object.assign({
            cancelToken: cancelToken
          }, restConfig));
          break;

        default:
          promise = axios_1.default.request({
            url: reqUrl,
            method: method,
            data: params,
            cancelToken: cancelToken
    ...
    promise.then(function (response) {
      ...
      if (axios_1.default.isCancel(error)) {
        return reject(config_1.REQ_ERR.CANCELED);
      } // 超时


      if (error.message.toLowerCase().includes('timeout')) {
        return reject(config_1.REQ_ERR.TIMEOUT);
      } // 网络错误


      if (error.message.toLowerCase().includes('network')) {
        return reject(config_1.REQ_ERR.NETWORK);
      }
...
function requestCreator(_temp) {
  ...
  var post = isFormData ? function (url, params, config) {
    return request(url, qs_1.default.stringify(params), Object.assign({
      cache: false,
      baseUrl: baseUrl
    }, config, {
...
var cancelRequest = function cancelRequest(cancelKey) {
...
exports.cancelRequest = cancelRequest; // 超时自动重传

axios_1.default.interceptors.response.use(function (response) {
  return response;
}, function (err) {
  var _err$message, _headers$config_1$REQ;
  ...
  headers[config_1.REQ_RETRY.FIELD] = (_headers$config_1$REQ = headers[config_1.REQ_RETRY.FIELD]) != null ? _headers$config_1$REQ : 0;

  if (headers[config_1.REQ_RETRY.FIELD] >= config_1.REQ_RETRY.COUNT) {
    return Promise.reject(err);
  }

  headers[config_1.REQ_RETRY.FIELD] += 1;
  var backOff = new Promise(function (resolve) {
    setTimeout(resolve, config_1.REQ_RETRY.INTERVAL);
  });
  return backOff.then(function () {
    return (0, axios_1.default)(err.config);
  });
});